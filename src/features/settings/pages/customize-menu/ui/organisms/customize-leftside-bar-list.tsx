...
import {
  ShortCutLinksType,
  SHORT_CUT_LINKS_LIMIT_SIZE,
} from "../../../../../../shared/consts";
import { popUpMessageModel } from "../../../../../../entities/pop-up-message";

const CustomizeLeftsideBarList = () => {
  const { setting, change } = useSettings<number[]>("menu");
  const { setting: shortCutMenu, change: shortCutChange } =
    useSettings<ShortCutLinksType>("shortCutLinks");

  const enabledLeftsideBarItems = getChosenRoutes(setting);
  const enabledShortCutMenu = useMemo(
    () => getChosenRoutes(shortCutMenu),
    [shortCutMenu]
  );
